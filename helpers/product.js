...
export const ProductHelpers = {
    ...
    // helpers/ProductHelpers.js - AGREGAR ESTE MÉTODO
    validateBarcodeUniqueness: async (barcode, { req }) => {
        if (!barcode) return true; // Si no se envía barcode, no validar

        const barcodeNumber = Number(barcode);
        if (isNaN(barcodeNumber)) throw new Error('Código de barras inválido');

        // Buscar si existe otro producto con el mismo barcode
        const existingProduct = await Producto.findOne({
            barcode: barcodeNumber,
            _id: { $ne: req.params.id } // Excluir el producto actual
        });

        if (existingProduct) {
            throw new Error(`El código de barras ${barcode} ya está en uso por otro producto`);
        }
